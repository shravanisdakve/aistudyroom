...
// Teacher/Professor Navigation
const teacherNavigation = [
    { name: 'Overview', href: '/teacher-dashboard', icon: LayoutDashboard },
    { name: 'Course Management', href: '/create-course', icon: ClipboardList },
    { name: 'Assignments', href: '/create-assignment', icon: FileText },
    { name: 'Auto-Grader', href: '/auto-grader', icon: CheckCircle },
    { name: 'Lesson Planner', href: '/lesson-planner', icon: BrainCircuit },
    { name: 'AI Co-Pilot', href: '/tutor', icon: MessageSquare },
];
        ...
        <> {/* Added Fragment to wrap sidebar and modal */}
            <aside className="w-64 flex-shrink-0 bg-slate-800/50 p-6 flex flex-col ring-1 ring-slate-700"> {/* Added ring */}
                ...
                <nav className="flex-1 space-y-2">
                    ...
                    {(currentUser?.role === 'teacher' ? teacherNavigation : studentNavigation).map((item) => (
                        <NavLink
                            key={item.name}
                            to={item.href}
                            end={item.href === '/' || item.href === '/teacher-dashboard'}
                            className={({ isActive }) =>
                                `flex items-center px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 ${isActive
                                    ? currentUser?.role === 'teacher'
                                        ? 'bg-emerald-600 text-white shadow-lg' // Emerald for teachers
                                        : 'bg-violet-600 text-white shadow-lg'   // Violet for students
