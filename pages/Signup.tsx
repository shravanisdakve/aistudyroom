...
const Signup: React.FC = () => {
  ...
  const [primarySubject, setPrimarySubject] = useState(''); // Added primarySubject state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === 'teacher' && !primarySubject) {
      setError('Please enter your primary subject or department.');
      return;
    }
    ...
    setLoading(true);
    try {
      await signup(displayName, email, university, password, role, primarySubject);
      navigate('/');
    } catch (err: any) {
    ...
    <div className="w-full max-w-md p-8 space-y-8 bg-slate-800/50 rounded-xl ring-1 ring-slate-700">
      ...
      <div className="flex gap-4 mb-6">
        <button
          ...
          className={`flex-1 p-3 rounded-lg border text-sm font-semibold transition-all ${role === 'student'
            ? 'bg-violet-600 border-violet-500 text-white ring-2 ring-violet-500/50'
        ...
        <button
          ...
          className={`flex-1 p-3 rounded-lg border text-sm font-semibold transition-all ${role === 'teacher'
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="university" className="block text-sm font-medium text-slate-300 mb-2">Institution / College / School</label>
          <Input
            id="university"
            type="text"
            value={university}
        ...
        {/* Conditionally render Primary Subject for Teachers */}
        {role === 'teacher' && (
          <div className="animate-in fade-in slide-in-from-top-2">
            <label htmlFor="primarySubject" className="block text-sm font-medium text-slate-300 mb-2">Primary Subject / Department</label>
            <Input
              id="primarySubject"
              type="text"
        ...
        <div>
          <label htmlFor="email-signup" className="block text-sm font-medium text-slate-300 mb-2">Email Address {role === 'teacher' && <span className="text-slate-500 text-xs font-normal">(Institutional preferred)</span>}</label>
          <Input
            id="email-signup"
            type="email"
            value={email}
      ...
      <p className="text-sm text-center text-slate-400">
        ...
        {/* Optional Complete Profile Link could go conceptually here, but user asked for it AFTER signup */}
      </p>
    </div>
  );
};
